    ...
    if st.session_state.df is None:
        ...
        if viz_type == "Distribution":
            ...
            if numeric_cols:
                ...
                with col2:
                    # KDE plot using scipy (properly indented inside the if block)
                    try:
                        ...
                        # Fallback to histogram if any error occurs
                        fig = px.histogram(df, x=selected_col, nbins=30, 
                                         title=f"Histogram of {selected_col}",
                                         opacity=0.7, color_discrete_sequence=['blue'])
                        st.plotly_chart(fig, use_container_width=True)
                        st.error(f"Could not generate KDE plot: {e}")
                
                # Statistics (outside the col2 block, inside the numeric_cols check)
                stats_summary = df[selected_col].describe()
                st.write("**Statistics:**")
                st.write(f"Mean: {stats_summary['mean']:.2f} | Median: {df[selected_col].median():.2f} | Std: {stats_summary['std']:.2f}")
            else:
